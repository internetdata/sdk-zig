...
function main() {
    # ALWAYS, before anything else. Condition 1 above is the state this repo is
    # actually in, so without this the resolver gate would only ever be seen
    # taking its skip path - and a gate seen only skipping is not a gate.
    selfTestGate
    ...
    if [ -n "$LOCAL_PATH" ] ; then
        saveLocalDependency "$LOCAL_PATH"
    else
        zigRun fetch --save=internetdata "git+${REPO_URL}#v${newest}"
...
# failure is SILENT: every test passes, against the wrong code. A path dependency
# carries no hash and names no tag, so a url plus a hash naming the tag we asked
# for is the proof that `zig fetch` went to the network.
#
# `zig fetch --save' does NOT store the url it was handed: it rewrites it to
# `?ref=<tag>#<commit>', recording both the tag asked for and the commit that tag
# resolved to. Matching the `#v<tag>' form that went IN therefore never matches,
# which is a gate that can only fail closed - and before the first tag exists,
# nothing would ever reach it to notice. selfTestGate exercises both forms.
...
# Proves assertFromATag both accepts and REFUSES, on synthetic manifests, every
# run. Costs nothing, needs no network, and is the only thing standing between a
# resolver gate and a gate nobody has ever seen do its job.
function selfTestGate() {
    local dir rc=0
    dir="$(mktemp -d)"
    gateCase "$dir" pass 'the form zig fetch --save actually writes' \
        "$(resolved 1.0.0)" || rc=1
    gateCase "$dir" fail 'a path dependency onto the working tree' \
        '.path = "..",' || rc=1
    # The state that makes the path check load-bearing rather than redundant: a
    # local run inserted into a manifest a previous `--save' had already filled
    # in. Every other check passes on this, and the build takes the path.
    gateCase "$dir" fail 'a path dependency beside a stale url and hash' \
        ".path = \"..\",
            $(resolved 1.0.0)" || rc=1
    gateCase "$dir" fail 'a url naming a different tag' \
        "$(resolved 0.9.0)" || rc=1
    gateCase "$dir" fail 'a dependency with no hash' \
        ".url = \"$(resolvedUrl 1.0.0)\"," || rc=1
    # The shape a naive gate expects, and the reason this self-test exists: it is
    # what goes IN, never what comes back out, so a gate matching it passes
    # nothing and fails everything.
    gateCase "$dir" fail 'the pre-resolution #v<tag> form' \
        ".url = \"git+${REPO_URL}#v1.0.0\",
            .hash = \"internetdata-1.0.0-AAAAAAAAAAAA\"," || rc=1
    rm -rf "$dir"
    if [ "$rc" -ne 0 ] ; then
        echo "==> the tag resolver gate does not work; refusing to report on a release" >&2
        exit 1
    fi
    echo "==> tag resolver gate self-checked (6 cases)"
}

# A dependency as `zig fetch --save' leaves it: the tag it was ASKED for and the
# commit that tag RESOLVED to, plus the hash that pins the fetched tree.
function resolved() {
    printf '.url = "%s",\n            .hash = "internetdata-%s-AAAAAAAAAAAA",' \
        "$(resolvedUrl "$1")" "$1"
}

function resolvedUrl() {
    echo "git+${REPO_URL}?ref=v${1}#0123456789abcdef0123456789abcdef01234567"
}

function gateCase() {
    local dir="$1" want="$2" name="$3" dependency="$4" got="pass"
...
EOF
    ( cd "$dir" && assertFromATag "1.0.0" ) >/dev/null 2>&1 || got="fail"
    if [ "$got" != "$want" ] ; then
        echo "GATE SELF-TEST FAILED: ${name} should ${want}, it ${got}ed" >&2
        return 1
    fi
    return 0
}
...
function reportKey() {
    ...
    # empty string, so the variable is SET and a plain unset check never fires,
    # while an empty key is sent as no key at all.
