...
//! this is how a Zig suite gets what a recording transport gives the other
//! languages: whether the key reached the wire, how many requests one call took,
...
//!
//! The presigned transfer does not pass through here: the `Location` a 302
//! carries is absolute, so the library goes straight to object storage. That
...
pub const Fact = struct {
    /// Path only. A query string carries the database id and the format, so it
    /// is dropped before anything is stored.
    path: []const u8,
    /// Whether the request carried the suite's key. Without this a run is
    /// indistinguishable from an unauthenticated one, and every assertion about
    /// what the key may see is vacuous.
    carried_key: bool,
    status: u16,
};
...
pub const Proxy = struct {
    gpa: Allocator,
    io: Io,
    /// The upstream origin, e.g. `https://staging.internetdata.io`.
    upstream: []const u8,
    ...
    /// No answer from these endpoints is big. The database itself never passes
    ...
    /// The untouched answer to `path`, which is the only way to see what the
    /// server actually served rather than what the typed model kept.
...
///
/// Redirects are NOT followed: the 302 the download endpoint answers with is the
/// thing under test, and it has to reach the library.
fn forward(self: *Proxy, target: []const u8, authorization: []const u8) !Answer {
    ...
    var request = try client.request(.GET, try std.Uri.parse(url), .{
        ...
        .headers = .{
            ...
            .user_agent = .{ .override = "internetdata-zig-integration" },
        },
    });
