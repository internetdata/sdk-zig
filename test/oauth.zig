...
test "the poll follows every corpus case" {
    ...
    for (data.value.oauth.poll.cases) |case| {
        ...
        var client = try internetdata.Client.init(gpa, FakeClock.install(harness.io()), .{
...
fn expectFailure(
    name: []const u8,
    err: internetdata.OauthCallError,
    diagnostics: *const internetdata.Diagnostics,
    ...
    if (std.mem.eql(u8, got, "client")) {
        try std.testing.expect(diagnostics.errorCode() == null);
        const ordinary: internetdata.CallError = switch (err) {
            error.OauthAccessDenied, error.OauthExpiredToken, error.OauthRejected => unreachable,
            else => |e| e,
        };
        if (want.kind) |kind| {
            try std.testing.expectEqualStrings(kind, internetdata.kindName(ordinary));
        }
        if (want.retryable) |retryable| {
            try std.testing.expectEqual(retryable, internetdata.isRetryable(ordinary));
        }
    }
}
...
fn callOperation(client: *internetdata.Client, operation: []const u8, args: corpus.OauthArgs) !void {
    var diagnostics: internetdata.Diagnostics = .{};
...
) internetdata.OauthCallError!void {
    const oauth = client.oauth();
    const id = args.clientId orelse client_id;
    const options: internetdata.OauthOptions = .{ .diagnostics = diagnostics };
